...
# Usage:
#   bin/rails runner scripts/import_ableton_once.rb

ABLETON_FEED = "https://www.ableton.com/en/blog/feeds/latest/"
...
def extract_og_image_from_page(url)
  ...
  # Fallback: pick first large-ish <img> from page content
...
def import_ableton_once
  xml = fetch_xml(ABLETON_FEED)
  items = feed_items(xml)
  raise "No items in feed" if items.empty?
  ...
  items.each do |item|
    ...
    link = strip_tracking(link.presence || item.at_xpath("*[local-name()='link']/@href")&.text)
    next if link.blank?
    ...
    puts "ableton => #{article.id}: #{article.title}"
    puts "http://localhost:4000#{article.path}"
    puts ""
    return
  end

  puts "No non-duplicate items found."
end
